...
AI Memory replaces that pattern with a disciplined primitive. Every save writes an audit-log entry with a SHA-256 input hash. The linked entry survives the daily pruner, so months later you can click any memory and see exactly which conversation produced it, who saved it, and what the agent's permission state was at the time. The content of the memory never lands in the audit log in plain text - only the hash.
...
> **One primitive, three jobs.** AI Memory replaces three things that used to be separate concepts: custom instructions, skills, and long-term memory. A memory with `status=always` is what other tools call a "custom instruction." A memory with a `slug` is what other tools call a "skill." Plain memories are memories. Less to learn, one place to audit.
