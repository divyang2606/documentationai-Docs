...
<Columns cols="3">
  <Card title="Safe Mode" href="#safe-mode" icon="shield">
    One toggle makes every ability read-only instantly. Your AI can read files, check themes, and audit your database - but cannot write, edit, or execute anything.
  </Card>

  <Card title="Ability Manager" href="#ability-manager" icon="sliders">
    Every one of the 175+ abilities can be enabled or disabled individually. Turn off entire categories when you don't need them. Less surface area means less risk.
  </Card>

  <Card title="No Anonymous Access" href="#authentication" icon="key">
    Every connection requires a WordPress Application Password over HTTPS. Only Administrator-role users can connect. Generate and revoke passwords anytime from **SproutOS > MCP Connect**.
  </Card>

  <Card title="Protected Files" href="#whats-always-protected" icon="lock">
    Core files are always blocked regardless of which abilities are on. `.env`, `wp-config.php`, `.htaccess`, and core WordPress directories cannot be read, written, or deleted.
  </Card>
  ...
  <Card title="Admin Bar Indicator" href="#the-admin-bar-indicator" icon="monitor">
    Your WordPress admin bar always shows whether SproutOS is active. Visible on every admin page so you always know the current state at a glance.
  </Card>
</Columns>
