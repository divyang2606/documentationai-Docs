...
<Columns cols="2">
  ...
  <Card title="Protected Files" href="#what-files-are-always-blocked" icon="lock">
    Core files are always blocked regardless of which abilities are on. `.env`, `wp-config.php`, `.htaccess`, and core directories cannot be touched.
  </Card>

  <Card title="Safety Controls" href="#what-other-safety-controls-are-available" icon="settings">
    Crash Guard auto-disables fatal files and activates Safe Mode. Rate Limiter, Audit Log, and Meta Snapshots are all configurable from Settings.
  </Card>
