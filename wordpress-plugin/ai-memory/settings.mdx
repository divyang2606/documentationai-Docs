...
When an agent calls `memory-save`, it must declare a `trigger_type` - the reason this save fired. The trigger maps to an initial confidence score:
...
| `repeated_correction` | 0.85 | The agent corrected the same behavior two or more times in a session. |
...
- **Above the threshold** - memory lands as `status=active` and is available to agents on the next session.
- **Below the threshold** - memory lands as `status=pending_review` and shows up in the badge at the top of the AI Memory tab. You approve or reject each one. If nothing happens within 7 days, the auto-promote cron flips it to Active so the queue doesn't stagnate.
...
Inferred saves are the ones most likely to be junk - the agent guessed that something was worth remembering without a clear user signal. SproutOS defaults them to 0.40 to route them into pending review under the Strict preset. A well-prompted agent rarely fires `inferred`. If your pending queue fills up with inferred saves, that's a signal the connected agent needs tighter prompting.
...
<Expandable title="What happens when I hit the hard cap of 1,000 memories?">
  SproutOS automatically archives the memory with the lowest confidence score that isn't pinned. This frees up one slot so the new save can proceed. If every memory in your library is pinned, the save is blocked and you'll see an error. The fix is to manually archive at least one pinned memory from the AI Memory tab.
</Expandable>
